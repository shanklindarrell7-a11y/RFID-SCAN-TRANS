...
void rfid_edit_name_callback(void* context) {
    RfidApp* app = context;
    RfidSignal* signal = &app->state->signals[app->state->selected_signal_index];
    
    // Update signal name
    strncpy(signal->name, app->state->text_input_buffer, MAX_SIGNAL_NAME - 1);
    signal->name[MAX_SIGNAL_NAME - 1] = '\0';
...
void rfid_scene_edit_name_on_enter(void* context) {
    ...
    strncpy(app->state->text_input_buffer, signal->name, MAX_SIGNAL_NAME - 1);
    app->state->text_input_buffer[MAX_SIGNAL_NAME - 1] = '\0';
    
    text_input_reset(app->text_input);
    text_input_set_header_text(app->text_input, "Enter Signal Name");
    text_input_set_result_callback(
        app->text_input,
        rfid_edit_name_callback,
        app,
